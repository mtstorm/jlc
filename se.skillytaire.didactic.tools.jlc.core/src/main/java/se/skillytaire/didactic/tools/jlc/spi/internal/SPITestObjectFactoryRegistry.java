...
public class SPITestObjectFactoryRegistry{
	private static TestObjectFactory<?>[] TEST_OBJECT_FACTORIES =
			ServiceLoader.load(TestObjectFactory.class)
			 .stream()
			 .map(ServiceLoader.Provider::get)
			 .filter(TestObjectFactory.class::isInstance)
			 .map(TestObjectFactory.class::cast)
			 .toArray(size -> new TestObjectFactory[size]);
