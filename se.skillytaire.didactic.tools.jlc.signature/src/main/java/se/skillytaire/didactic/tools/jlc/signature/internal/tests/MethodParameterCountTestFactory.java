...
public class MethodParameterCountTestFactory<N extends TestSignatureConfiguration<N,T,S,E,D>,T, S extends Signature,E extends Executable,D extends JLCFeatureConfiguration> implements SignatureTestFactory<N,T,S,E,D> {

	@Override
	public boolean matches(N configuration) {
		return configuration.getSignature().hasParameters() && !configuration.getSignature().isApi();
	}

	@Override
	public JLCTestNode<T> create(N configuration) {
		return new SignatureParameterCountTest<>(configuration);
	}
